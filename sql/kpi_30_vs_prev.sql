...
-- Window: last 30 days vs prior 30 days (based on MAX(date) in table)
...
agg AS (
  SELECT 'current' AS period, SUM(spend) AS spend, SUM(conversions) AS conv
  FROM base WHERE date BETWEEN start_curr AND end_date
  UNION ALL
  SELECT 'prior',   SUM(spend),            SUM(conversions)
...
FROM final curr
JOIN final prior ON prior.period = 'prior'
